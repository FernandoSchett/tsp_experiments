...
void Tour::two_opt(IData& inst, Parameters& params, bool& improvement) {
    //this->print_tour();
    ...
    while (l != this->tour.end() ) {
        while (l !=  this->tour.end()) {   
            ...
            if (delta > 0) {
                improvement = true;
                if(params.scheme == "best_improvement" && delta > best_delta){
                    j_best = j;
                    l_best = l;
                    best_delta = delta;
                    ...
                    //this->print_tour();
                    std::reverse(j, l);
                    this->sol_value = sol_value - delta;
    ...
    if(best_delta > 0){
        ...
        this->sol_value = sol_value - best_delta;
    }    
    this->tour.pop_back();
